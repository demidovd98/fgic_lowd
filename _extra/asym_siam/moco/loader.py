...
class CropsTransform:
    ...
    def __init__(
        ...
        self.enable_asymm = enable_asymm
    ...
    def __call__(self, x):
        ...
        if self.enable_scalemix: # combine 2 crops of one image
            img_source = scalemix(self.query_transform(x), self.query_transform(x),)
            crops.append(img_source)
        
        elif self.enable_multicrop: # 6 crops of 96x96
            #for img_source in range(6):
            for img_source in range(4):
                crops.append(self.query_mini_transform(x))

        elif self.enable_asymm: # strong augs on source (smaller crop), weak augs on tartget (larger crop)
            img_source = self.query_transform(x)
            crops.append(img_source)

            img_target = self.key_transform(x)
            crops.append(img_target)

        else: # basic augmentations
            img_source = self.query_transform(x)
            crops.append(img_source)
