...
class Dataset_Meta:
    def __init__(self, child, dataset_name, root, is_train=True, 
                 data_len=None, transform=None, vanilla=None, split=None, 
                 low_data=True, saliency=False, aug_type="none"):
        ...
        if self.low_data:
            ...
            if child.inter_low_path is None:
            ...
            if split is not None:
                if len(str(split)) >= 5:
                    ...
                    if self.is_train:
                        self.split = '50_50/3way_splits/' + str(split)[-1] + '/train_' + str(split) + '.txt'
                    else:
                        self.split = '50_50/3way_splits/' + str(split)[-1] + '/test_100_sp' + str(split)[-1] + '.txt'
                else:
                    if self.is_train:
                        self.split = 'train_' + str(split) + '.txt'
                    else:
                        self.split = 'test.txt'
            else:
                if self.is_train:
            ...
            self.ld_label_list = []
            ld_file = open(os.path.join(self.root, child.inter_low_path, self.split))
            ...
            for line in ld_file:
                ...
                if self.saliency:
                    self.file_list_full.append(os.path.join(self.root, child.inter_img_path, path))
            ...
            self.file_list = child.file_list
            self.file_list_full = child.file_list_full

            print(f"[INFO] Number of {self.set_name} samples:" , len(self.file_list), f", number of {self.set_name} labels:", len(child.label_list))
        ...
        #i = 0
        for id_f, file in enumerate(self.file_list[:data_len]):
            img_temp = scipy.misc.imread(os.path.join(self.root, child.inter_img_path, file))
            ...
            if self.saliency:
                ...
                if self.dataset_name == "CUB" and (child.bound_box_parts):
                    ## parts:
                ...
                if self.dataset_name == "CUB" and (child.bound_box):
                    ## bbox:
                    #if self.bound_box:
            ...
            else: # basically center crop with 90%x90% of img
            ...
            if self.saliency:
                ...
                if ( (img_temp.shape[0] != img_mask.shape[0]) or (img_temp.shape[1] != img_mask.shape[1]) ):
                    print("_____Wrong Index is:", id_f)
        ...
        if not self.low_data:
            if self.is_train:
                if self.dataset_name == "CUB":
                    self.label = [x for i, x in zip(child.train_test_list, child.label_list) if i][:self.data_len]
                ...
                if self.dataset_name == "CUB":                    
                    self.label = [x for i, x in zip(child.train_test_list, child.label_list) if not i][:self.data_len]
                elif self.dataset_name == "cars":
                    raise NotImplementedError()
                elif self.dataset_name == "air":
                    raise NotImplementedError()
    ...
    def __getitem__(self, index):
        ...
        if self.is_train:
            ...
            #saliency_check = False #False
            if self.saliency:
                img, target, imgname, mask = self.img[index], self.label[index], self.img_name[index], self.mask[index]
            else:
                img, target, imgname = self.img[index], self.label[index], self.img_name[index]
...
class CUB(Dataset_Meta):
    ...
    def __init__(self, root, inter_low_path=None, inter_img_path=None, is_train=True, data_len=None, transform=None, # general class parameters
                 ...
                 bound_box=False, bound_box_parts=False # class-specific parameters
                 ):
        ...
        if low_data:
            if inter_low_path is not None:
                ...
                self.inter_low_path = 'low_data/CUB200/image_list/'
        ...
        #if not self.low_data: 
        train_test_file = open(os.path.join(root, 'train_test_split.txt'))

        if self.bound_box:
            bounding_boxes_file = open(os.path.join(root, 'bounding_boxes.txt'))
        if self.bound_box_parts:
            parts_file = open(os.path.join(root, 'parts/part_locs.txt'))
        ...
        for line in label_txt_file:
            self.label_list.append(int(line[:-1].split(' ')[-1]) - 1)
        ...
        if not low_data:
            if is_train:
                ...
                self.file_list_full = [ os.path.join(self.root, 'images', x)  for i, x in zip(self.train_test_list, img_name_list) if i]
            else:
                self.file_list = [x for i, x in zip(self.train_test_list, img_name_list) if not i]
                self.file_list_full = [ os.path.join(self.root, 'images', x)  for i, x in zip(self.train_test_list, img_name_list) if not i]
        ...
        super().__init__(child=self, dataset_name="CUB", root=root, is_train=is_train, data_len=data_len, transform=transform, 
                         vanilla=vanilla, split=split, low_data=low_data, saliency=saliency, aug_type=aug_type
                         )
...
class CarsDataset(Dataset_Meta):
    ...
    def __init__(self, root, inter_low_path=None, inter_img_path=None, is_train=True, data_len=None, transform=None, # general class parameters
                 vanilla=None, split=None, low_data=True, saliency=False, aug_type="none", # general class parameters
                 cleaned=None # class-specific parameters
                ):
